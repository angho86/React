...
export const registrationSchema = z.object({
    email: z
        ...
        .min(7, { message: "Email should contain at least 7 symbols"})
        .max(120, { message: "Email should not be longer then 120 symbols"}),

        username: z
        .string({message: "Username should be a string" })
        .email({ message: "Username should be in a valid format"})
        .min(3, { message: "Username should contain at least 3 symbols"})
        .max(60, { message: "Username should not be longer then 60 symbols"}),
        password: z
